...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const newad = await axios.post(`${config.endpoint}/user/addresses`,{address:newAddress.value}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      // setNewAddress( (current)=>({...current , isAddingNewAddress:false}));  -----optional
      setNewAddress({value:'' , isAddingNewAddress:false});
      setAddresses({...addresses , all: newad.data})
      return addresses
      // console.log(newad.address.data)
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const del = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,{
        headers:{
          Authorization: `Bearer ${token}`,
        },
      })
      setAddresses({...addresses ,all:del.data});
    ...
    if(validateRequest(items,addresses)){
      try{
        ...
        if(error.response){
          ...
          enqueueSnackbar(' Check that the backend is running, reachable and returns valid JSON.',{variant:'error'})
        }
        return false;
      }
    }
    ...
    <>
      ...
      <Grid container>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          ...
          <Box>
            <Stack  className='order-details' display='flex' flexDirection="column" justifyContent="space-between" bgcolor="#E9F5E1">
            <h3>Order Details</h3>
              <div class="order">
              <div>Products</div>
